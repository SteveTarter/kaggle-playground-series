...
class ExperimentSetup:
    ...
    def read_dataset(self, dataset_name) -> pd.DataFrame:
        if dataset_name == 'training':
            data_dir = Path('/kaggle/input/competitions/playground-series-s6e4') if self.running_in_kaggle() else Path('data')
            df = pd.read_csv(data_dir / 'train.csv')
        elif dataset_name == 'test':
            data_dir = Path('/kaggle/input/competitions/playground-series-s6e4') if self.running_in_kaggle() else Path('data')
            df = pd.read_csv(data_dir / 'test.csv')
        elif dataset_name == 'original':
            data_dir = Path('/kaggle/input/datasets/miadul/irrigation-water-requirement-prediction-dataset') if self.running_in_kaggle() else Path('original_data')
            df = pd.read_csv(data_dir / 'irrigation_prediction.csv')

...
class ExperimentSetup:
    ...
    """
    def __init__(self, seed=10301, model_name=None, target='PitNextLap'):
        self.seed = seed
    ...
    def read_dataset(self, dataset_name) -> pd.DataFrame:
        if dataset_name == 'training':
            data_dir = Path('/kaggle/input/competitions/playground-series-s6e5') if self.running_in_kaggle() else Path('data')
            df = pd.read_csv(data_dir / 'train.csv')
        elif dataset_name == 'test':
            data_dir = Path('/kaggle/input/competitions/playground-series-s6e5') if self.running_in_kaggle() else Path('data')
            df = pd.read_csv(data_dir / 'test.csv')
        elif dataset_name == 'original':
            data_dir = Path('/kaggle/input/datasets/aadigupta1601/f1-strategy-dataset-pit-stop-prediction') if self.running_in_kaggle() else Path('original_data')
            df = pd.read_csv(data_dir / 'f1_strategy_dataset_v4.csv')

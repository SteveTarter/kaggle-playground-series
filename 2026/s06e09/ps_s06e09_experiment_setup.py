...
class ExperimentSetup:
    """
    Handles global configuration, seeding, and environment setup for PS-S06E09.
    """
    def __init__(
        self,
    ...
    def read_dataset(self, dataset_name: str) -> pd.DataFrame:
        ...
        if dataset_name == 'training':
            data_dir = Path('/kaggle/input/competitions/playground-series-s6e9') if self.running_in_kaggle() else Path('data')
            ...
            data_dir = Path('/kaggle/input/datasets/itzzomkar/ev-adoption-behavior-and-range-anxiety') if self.running_in_kaggle() else Path('original_data')
            if (data_dir / 'EV_Adoption_and_Range_Anxiety_Dataset.csv').exists():
                df = pd.read_csv(data_dir / 'EV_Adoption_and_Range_Anxiety_Dataset.csv')
            elif (data_dir / 'original.csv').exists():
                ...
                if csv_files:
                    ...
                    raise FileNotFoundError(f"Original dataset not found in {data_dir}")
        elif dataset_name == 'pseudo':
            file_path = Path('/kaggle/input/notebooks/stephentarter/ps-s06e09-pseudo-labeling/data/train_pseudo.csv') if self.running_in_kaggle() else Path('data/train_pseudo.csv')
    ...
    def load_rfe(self, rfe_filename: str, X: pd.DataFrame) -> pd.Index:
        path = Path('/kaggle/input/datasets/stephentarter/ps-s06e09-artifacts') / rfe_filename \
    ...
    def load_optuna_params(self, optuna_filename: str) -> dict:
        path = Path('/kaggle/input/datasets/stephentarter/ps-s06e09-artifacts') / optuna_filename \
        ...
        if not self.running_in_kaggle():
            ...
            try:
                ...
                metadata_cmd = [
                    ...
                    "datasets", "metadata",
                    "stephentarter/ps-s06e09-artifacts",
                    "-p", staging_dir
                ]
    ...
    def save_probabilities(
        ...
        # Column names: prob_0 (No) and prob_1 (Yes / Will_Buy_EV)

...
class ModelVisualizer:
    """
    Centralizes all visualization logic for Gradient Boosting and Neural Network models in PS-S06E07.
    """
    ...
    def plot_learning_curves(self, eval_results, metric='mlogloss', metric_period=1):
        """
        ...
        # Extended candidates for various metrics
        candidates = [
            metric, metric.upper(), metric.lower(),
            'mlogloss', 'multi_logloss', 'RMSE', 'rmse', 'RootMeanSquaredError', 'l2', 'auc', 'TotalF1', 'balanced_accuracy'
        ]
        ...
        for i, result in enumerate(eval_results):
            ...
            if val_data is not None:
                # The validation array length * metric_period = total iterations
            ...
            if train_data is not None:
                # Match the training data to the SAME total iteration range 
                x_train = np.linspace(0, total_iters, len(train_data))
    ...
    def plot_multiclass_roc_curve(self, y_true, y_score, title='Multiclass ROC Curve (One-vs-Rest)'):
        """
        ...
        Args:
            ...
            y_score: Target scores, can either be probability estimates of the positive class,
                     confidence values, or non-thresholded measure of decisions.
        """
        # Mapping for descriptive legend (at-risk=0, unhealthy=1, fit=2)
        class_names = {0: "at-risk", 1: "unhealthy", 2: "fit"}
